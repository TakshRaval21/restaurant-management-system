...
class _SResult {
  ...
  const _SResult({
    ...
    required this.targetIndex, required this.icon, required this.color,
  });
}
...
const _navItems = [
  ...
  _NavItem('Restaurant Settings', Icons.store_outlined,            AppRoutes.restaurantSettings),
  _NavItem('Tables',              Icons.table_restaurant_outlined, AppRoutes.tableManagement),
...
class _AdminLayoutState extends State<AdminLayout> {
  ...
  String _fmtRole(String r) => const {
    'owner':   'Owner',
    ...
    _notifChannel = _sb.channel('notif-$_restaurantId')
      ..onPostgresChanges(
        ...
        filter: PostgresChangeFilter(
            ...
            column: 'restaurant_id', value: _restaurantId!),
        callback: (p) async {
          if (!mounted) return;
          final o = p.newRecord;
          ...
          if (tableId != null) {
            final t = await _sb.from('tables').select('table_number')
          ...
          _addNotif(_Notif(
            ...
            body:  'Order #${o['order_number']} · ${_typeLabel(o['order_type'] ?? 'dine_in')}',
            time:  DateTime.now(),
        ...
        callback: (p) {
          ...
          if (status == 'ready') {
            _addNotif(_Notif(
                ...
                body: 'Ready to serve', time: DateTime.now(),
                icon: Icons.check_circle_outline, color: AppColors.green));
          } else if (status == 'cancelled') {
            _addNotif(_Notif(
                id: '${DateTime.now().millisecondsSinceEpoch}',
                title: '❌ Order #$num Cancelled',
                body: 'Cancelled by staff', time: DateTime.now(),
                icon: Icons.cancel_outlined, color: AppColors.red));
          }
        },
      )
      ..subscribe();
  ...
  void _onSearch() {
    ...
    if (q.isEmpty) {
      if (mounted) setState(() { _showSearch = false; _results = []; });
      return;
    }
    _debounce?.cancel();
  ...
  Future<void> _runSearch(String q) async {
    ...
    try {
      ...
      final ts = await _sb.from('tables')
          .select('table_number, section, status')
          .eq('restaurant_id', _restaurantId!)
          .ilike('section', '%$q%').limit(3);
      for (final t in ts as List) {
        res.add(_SResult(
            label: 'Table ${t['table_number']}',
            subtitle: '${t['section'] ?? 'Main Hall'} · ${(t['status'] as String).toUpperCase()}',
            icon: Icons.table_restaurant_outlined, color: AppColors.orange,
            targetIndex: _routeIndex[AppRoutes.tableManagement]!));
      }
      if (num != null) {
        final tn = await _sb.from('tables')
            .select('table_number, section, status')
            .eq('restaurant_id', _restaurantId!).eq('table_number', num).limit(3);
      ...
      for (final e in em as List) {
        res.add(_SResult(
            ...
            icon: Icons.badge_outlined, color: AppColors.purple,
  ...
  void _closeAll() {
    if (!mounted) return;
    setState(() { _showNotifs = false; _showSearch = false; });
  ...
  Widget _buildTopBar(bool isMobile, bool isTablet, bool isDesktop) {
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        if (!isMobile) ...[
          Flexible(child: Text(_restaurantName,
          ...
          child: Container(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: isMobile ? 'Search...' : 'Search orders, tables, menu...',
                ...
                suffixIcon: _searchCtrl.text.isNotEmpty
                    ? IconButton(
                        icon: const Icon(Icons.close, size: 15, color: AppColors.textLight),
                        onPressed: () {
                          _searchCtrl.clear();
                          if (mounted) setState(() { _showSearch = false; _results = []; });
                          _searchFocus.unfocus();
        ...
        GestureDetector(
          onTap: () {
            if (!mounted) return;
            setState(() { _showNotifs = !_showNotifs; _showSearch = false; _searchFocus.unfocus(); });
          },
          child: Stack(clipBehavior: Clip.none, children: [
            AnimatedContainer(
              ...
              width: 36, height: 36,
              decoration: BoxDecoration(
                color: _showNotifs ? AppColors.primary.withOpacity(0.1) : AppColors.contentBg,
                borderRadius: BorderRadius.circular(10),
                border: Border.all(color: _showNotifs ? AppColors.primary : AppColors.border),
              ),
              child: Icon(
                  _showNotifs ? Icons.notifications : Icons.notifications_outlined,
                  size: 18,
          ...
          child: MouseRegion(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              ...
              if (isDesktop) ...[
                ...
                const Icon(Icons.expand_more, size: 14, color: AppColors.textMid),
              ],
            ]),
          ),
        ),
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                Padding(
                  ...
                  child: Column(children: [
                    _PRow(Icons.email_outlined, 'Email', _userEmail.isEmpty ? '—' : _userEmail),
...
class _SidebarContent extends StatelessWidget {
  ...
  const _SidebarContent({
    required this.selectedIndex, required this.expanded,
    required this.restaurantName, required this.onNavigate,
      ...
      child: Column(children: [
        ...
        Expanded(
          child: ListView.builder(
            padding: EdgeInsets.symmetric(horizontal: expanded ? 8 : 4, vertical: 4),
          ...
          child: Container(
            ...
            child: expanded
                ? Row(children: [
                    ...
                    const SizedBox(width: 9),
                    Text('Sign out',
                        style: TextStyle(color: AppColors.textSidebar.withOpacity(0.6), fontSize: 13)),
                  ])
          ...
          child: widget.expanded
              ? Row(children: [
                  ...
                  Text(widget.item.label,
                      style: active ? AppText.sidebarItemActive : AppText.sidebarItem),
                ])
              : Center(child: Icon(widget.item.icon, size: 20,
                  color: active ? Colors.white : AppColors.textSidebar)),
        ),
      ),
    );
    if (!widget.expanded) {
      return Tooltip(message: widget.item.label, preferBelow: false, child: tile);
    }
    return tile;
  }
}
...
class _SearchDropdown extends StatelessWidget {
  ...
  const _SearchDropdown({required this.results, required this.loading, required this.query, required this.onSelect});

  @override
  Widget build(BuildContext context) => Material(
    elevation: 14, borderRadius: BorderRadius.circular(12),
    child: Container(
          ...
          : results.isEmpty
              ? Padding(
                  ...
                  child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    const Icon(Icons.search_off, size: 17, color: AppColors.textLight),
                    const SizedBox(width: 8),
                    ...
                    return InkWell(
                      ...
                      child: Padding(
                        ...
                        child: Row(children: [
                          ...
                          const Icon(Icons.arrow_forward_ios, size: 10, color: AppColors.textLight),
                        ]),
                      ),
                    );
                  }),
...
class _NotifPanel extends StatelessWidget {
  ...
  const _NotifPanel({required this.notifs, required this.onMarkAll,
      required this.onDismiss, required this.onTap, required this.onViewAll});
  int get _unread => notifs.where((n) => !n.read).length;

  @override
  Widget build(BuildContext context) => Material(
    elevation: 16, borderRadius: BorderRadius.circular(14),
    child: Container(
        ...
        Flexible(
          child: notifs.isEmpty
              ? const Padding(
                  ...
                  child: Column(mainAxisSize: MainAxisSize.min, children: [
                    Icon(Icons.notifications_none_outlined, size: 36, color: AppColors.textLight),
                    SizedBox(height: 8),
        ...
        if (notifs.isNotEmpty)
          InkWell(
            ...
            child: Container(
              ...
              child: const Center(child: Text('View All Orders →',
...
class _NotifTile extends StatelessWidget {
  ...
  const _NotifTile({required this.n, required this.onTap, required this.onDismiss});
  ...
  Widget build(BuildContext context) => InkWell(
    ...
    child: Container(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
            width: 34, height: 34,
            decoration: BoxDecoration(color: n.color.withOpacity(0.1), borderRadius: BorderRadius.circular(8)),
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(n.body, style: const TextStyle(fontSize: 11, color: AppColors.textMid)),
          const SizedBox(height: 2),
          Text(_ago(n.time), style: const TextStyle(fontSize: 10, color: AppColors.textLight)),
        ])),
        const SizedBox(width: 6),
        GestureDetector(onTap: onDismiss,
            child: const Icon(Icons.close, size: 13, color: AppColors.textLight)),
      ]),
    ),
  );
}
...
class Responsive {
  ...
  static bool isTablet(BuildContext ctx)  => MediaQuery.of(ctx).size.width >= 600 && MediaQuery.of(ctx).size.width < 1100;
  ...
  static int gridCount(BuildContext ctx, {int mobile = 1, int tablet = 2, int desktop = 3}) =>
...
class _PRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
    ...
    const SizedBox(width: 7),
    Text('$label: ', style: const TextStyle(fontSize: 11.5, color: AppColors.textMid, fontWeight: FontWeight.w500)),
...
class _PAction extends StatelessWidget {
  ...
  const _PAction(this.icon, this.label, this.onTap, {this.color = AppColors.textDark});
  @override
  Widget build(BuildContext context) => InkWell(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(8),
          color: color == AppColors.red ? AppColors.red.withOpacity(0.05) : AppColors.contentBg),
      child: Row(children: [
        Icon(icon, size: 15, color: color),
        const SizedBox(width: 9),
        Text(label, style: TextStyle(fontSize: 12.5, color: color, fontWeight: FontWeight.w600)),
        const Spacer(),
        Icon(Icons.arrow_forward_ios, size: 10, color: color.withOpacity(0.4)),
      ]),
    ),
  );
}