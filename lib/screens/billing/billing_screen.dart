...
class _BillingScreenState extends State<BillingScreen>
  ...
  // ── FIX 2: Parse UTC timestamps correctly ──────────────────
  DateTime? _parseUtc(String? raw) {
    if (raw == null) return null;
    // Supabase sometimes omits the Z suffix — force UTC interpretation
  ...
  Future<void> _hydrateTableOrders() async {
    ...
    for (final order in orders) {
      ...
      if (orderId == null) { hydrated.add(order); continue; }

      List<Map<String, dynamic>>? items = widget.orderItemsCache?[orderId];
  ...
  Future<void> _load() async {
    final results = await Future.wait([
      _sb
          .from('payments')
          .select('*, orders(order_number, total_amount, tables(table_number))')
          .eq('restaurant_id', _restaurantId!)
          .order('paid_at', ascending: false)
          .limit(200),
      // ── FIX 1: include 'served' orders so they show in Pending Bills ──
      _sb
          .from('orders')
    ...
    final rawPayments = List<Map<String, dynamic>>.from(results[0]);
    ...
    // ── FIX 3: hydrate order_items if the join returned empty ──
    final rawReady = List<Map<String, dynamic>>.from(results[1]);
    ...
    setState(() {
      ...
      _completedBills = uniquePayments.where((p) => p['status'] == 'completed').toList();
      _readyOrders = hydratedReady; // ── FIX 3: use hydrated list ──
      _loading = false;
    });
  }
  ...
  double get _todayRevenue {
    ...
    return _payments.where((p) {
      final dt = _parseUtc(p['paid_at']); // ── FIX 2 ──
      return dt != null &&
          dt.year == today.year &&
          dt.month == today.month &&
          dt.day == today.day &&
          p['status'] == 'completed';
    }).fold(0.0, (s, p) => s + ((p['amount_paid'] as num?)?.toDouble() ?? 0));
  }

  int get _todayPaymentsCount {
    final today = DateTime.now();
    return _payments.where((p) {
      final dt = _parseUtc(p['paid_at']); // ── FIX 2 ──
      return dt != null &&
  ...
  // ── group parcel orders by customer name ──
  ...
  List<Map<String, dynamic>> _mergeItemsFromOrders(
    ...
    for (final order in orders) {
      ...
      for (final item in items) {
        ...
        final price = (item['item_price'] as num? ?? 0).toDouble();
        final qty = (item['quantity'] as int? ?? 1);
        final key = name.toLowerCase();
        if (merged.containsKey(key)) {
          merged[key]!['quantity'] = (merged[key]!['quantity'] as int) + qty;
          merged[key]!['total'] = (merged[key]!['total'] as double) + price * qty;
        } else {
          merged[key] = {
  ...
  Map<String, double> _calculateTotals(List<Map<String, dynamic>> mergedItems) {
    final subtotal = mergedItems.fold<double>(
        0.0, (sum, i) => sum + (i['total'] as double? ?? 0.0));
    final gst = double.parse((subtotal * _gstRate).toStringAsFixed(2));
    final grandTotal = double.parse((subtotal + gst).toStringAsFixed(2));
    return {'subtotal': subtotal, 'gst': gst, 'grandTotal': grandTotal};
  }
  ...
  Future<void> _generatePdfBill({
    ...
    final restName = _restaurantInfo?['name'] ?? 'Restaurant';
    final restAddr = _restaurantInfo?['address'] as String?;
    final restPhone = _restaurantInfo?['phone'] as String?;

    const darkTeal  = PdfColor.fromInt(0xFF004D40);
    const midTeal   = PdfColor.fromInt(0xFF00695C);
    const lightTeal = PdfColor.fromInt(0xFFE0F2F1);
    const bgStripe  = PdfColor.fromInt(0xFFF6FAF9);
    ...
    const divLine   = PdfColor.fromInt(0xFFE0E0E0);
    const white     = PdfColors.white;

    final isParcel = tableNumber.startsWith('📦');
    final pdfLabel = isParcel ? 'PARCEL' : 'TABLE';
    final pdfValue = isParcel
        ? tableNumber.replaceFirst('📦 ', '')
        : tableNumber;
    ...
    pdf.addPage(
      pw.Page(
        // ── FIX: Use A4 page format so it prints on the full page ──
        ...
        build: (pw.Context ctx) => pw.Center(
          // ── FIX: Center the receipt content on the A4 page ──
          child: pw.Container(
              ...
              children: [
                pw.Container(
                  ...
                  child: pw.Column(
                      ...
                      children: [
                    pw.Text(restName, textAlign: pw.TextAlign.center, style: bold(16, white)),
                    if (restAddr != null) ...[
                      pw.SizedBox(height: 4),
                      pw.Text(restAddr, textAlign: pw.TextAlign.center,
                          style: reg(7.5, const PdfColor.fromInt(0xFFB2DFDB))),
                    ],
                    if (restPhone != null) ...[
                      pw.SizedBox(height: 2),
                      pw.Text('Ph: $restPhone', textAlign: pw.TextAlign.center,
                          style: reg(7.5, const PdfColor.fromInt(0xFFB2DFDB))),
                    ],
                  ]),
                ),
                pw.Container(
                  ...
                  child: pw.Row(
                    ...
                    pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
                      pw.Text(pdfLabel, style: bold(6.5, midTeal)),
                      pw.SizedBox(height: 2),
                      pw.Text(pdfValue, style: bold(18, darkTeal)),
                    ]),
                    pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.end, children: [
                      pw.Text('DATE & TIME', style: bold(6.5, midTeal)),
                      pw.SizedBox(height: 2),
                      pw.Text(dtStr, style: bold(8, darkTeal)),
                      pw.Text(timeStr, style: reg(8, midTeal)),
                    ]),
                  ]),
                ),
                pw.Container(
                  color: const PdfColor.fromInt(0xFFF9FFFE),
                  padding: const pw.EdgeInsets.fromLTRB(12, 7, 12, 7),
                  child: pw.Text('Orders: ${orderIds.join('  •  ')}',
                      style: reg(7.5, textLight)),
                ),
                pw.Container(height: 0.5, color: divLine),
                pw.Container(
                  color: darkTeal,
                  padding: const pw.EdgeInsets.fromLTRB(12, 7, 12, 7),
                  child: pw.Row(children: [
                    pw.Expanded(flex: 10, child: pw.Text('ITEM', style: bold(7, white))),
                    pw.SizedBox(width: 20,
                        child: pw.Text('QTY', textAlign: pw.TextAlign.center, style: bold(7, white))),
                    pw.SizedBox(width: 36,
                        child: pw.Text('RATE', textAlign: pw.TextAlign.right, style: bold(7, white))),
                    pw.SizedBox(width: 44,
                        child: pw.Text('AMOUNT', textAlign: pw.TextAlign.right, style: bold(7, white))),
                  ]),
                ),
                ...mergedItems.asMap().entries.map((entry) {
                  final idx = entry.key;
                  final item = entry.value;
                  final bg = idx % 2 == 0 ? white : bgStripe;
                  ...
                  return pw.Container(
                    ...
                    child: pw.Row(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
                      pw.Expanded(flex: 10,
                          child: pw.Text(item['item_name'] as String, style: reg(9, textDark))),
                      pw.SizedBox(width: 20,
                          child: pw.Text('x${item['quantity']}',
                              textAlign: pw.TextAlign.center, style: reg(9, textMid))),
                      pw.SizedBox(width: 36,
                          child: pw.Text(_pdfMoney(price),
                              textAlign: pw.TextAlign.right, style: reg(8, textLight))),
                      pw.SizedBox(width: 44,
                          child: pw.Text(_pdfMoney(total),
                              textAlign: pw.TextAlign.right, style: bold(9, textDark))),
                    ]),
                  );
                }),
                pw.Container(height: 0.5, color: divLine),
                pw.Container(
                  color: const PdfColor.fromInt(0xFFF4F9F8),
                  padding: const pw.EdgeInsets.fromLTRB(12, 10, 12, 10),
                  child: pw.Column(children: [
                    pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
                      pw.Text('Subtotal', style: reg(9, textMid)),
                      pw.Text(_pdfMoney(totals['subtotal']!), style: bold(9, textDark)),
                    ]),
                    pw.SizedBox(height: 5),
                    pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
                     pw.Text('Tax @ ${RestaurantService.instance.taxRate.toStringAsFixed(0)}%', style: reg(9, textMid)),
                      pw.Text(_pdfMoney(totals['gst']!), style: bold(9, textDark)),
                    ]),
                  ]),
                ),
                pw.Container(height: 1, color: divLine),
                pw.Container(
                  color: darkTeal,
                  padding: const pw.EdgeInsets.fromLTRB(12, 12, 12, 12),
                  child: pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
                    pw.Text('GRAND TOTAL', style: bold(11, white)),
                    pw.Text(_pdfMoney(totals['grandTotal']!), style: bold(14, white)),
                  ]),
                ),
                pw.Container(
                  color: const PdfColor.fromInt(0xFFF4F9F8),
                  ...
                  child: pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
                    pw.Text('Payment Mode', style: reg(8.5, textMid)),
                    pw.Container(
                      padding: const pw.EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                      decoration: pw.BoxDecoration(
                          color: midTeal, borderRadius: pw.BorderRadius.circular(3)),
                      child: pw.Text(method.toUpperCase(), style: bold(8.5, white)),
                    ),
                  ]),
                ),
                pw.Container(height: 0.5, color: divLine),
                ...
                pw.Center(
                  child: pw.Column(children: [
                    pw.Text('* * *  THANK YOU  * * *',
                        textAlign: pw.TextAlign.center, style: bold(9, midTeal)),
                    pw.SizedBox(height: 4),
                    pw.Text('Please visit us again!',
                        textAlign: pw.TextAlign.center, style: reg(8, textLight)),
                  ]),
                ),
    ...
    await Printing.layoutPdf(
      ...
      name: 'Bill_${isParcel ? "Parcel" : "Table"}${pdfValue}_${DateFormat('yyyyMMdd_HHmm').format(now)}.pdf',
    );
  }
  ...
  Future<bool> _collectTablePayment({
    ...
    final orderIds = tableOrders.map((o) => o['id'] as String).toList();
    final tableId = tableOrders.first['table_id'] as String?;
    ...
    try {
      await _sb.from('payments').insert({
        ...
        'status': 'completed',
        'paid_at': DateTime.now().toUtc().toIso8601String(), // ── FIX 2: store as UTC with Z ──
  ...
  Future<bool> _markOrdersCompleted(List<String> orderIds) async {
    try {
      await _sb
          .from('orders')
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(children: [
          ...
          Text('$table Error', style: const TextStyle(fontSize: 16)),
        ]),
  ...
  // ── BILL DIALOG ──────────────────────────────────────────────
  Future<void> _showTableBillDialog(
      String tableNumber, List<Map<String, dynamic>> tableOrders) async {
    ...
    await showDialog(
      ...
      builder: (context) => StatefulBuilder(builder: (ctx, setDlg) {
        return Dialog(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                child: Row(children: [
                  ...
                  Expanded(
                      child: Column(
                          ...
                          children: [
                    Text(_restaurantInfo?['name'] ?? 'Restaurant',
                    ...
                    Text(
                      isParcel ? tableNumber : 'Table $tableNumber',
                      style: const TextStyle(
                  ...
                  Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                    Text(DateFormat('dd MMM yyyy').format(billOpenedAt),
                        style: const TextStyle(
                            color: Color(0xFFB2DFDB),
                            fontSize: 11,
                            fontWeight: FontWeight.w600)),
                    Text(DateFormat('hh:mm a').format(billOpenedAt),
                  ...
                  child: Column(
                      ...
                      child: Column(children: [
                        Container(
                          ...
                          child: const Row(children: [
                            Expanded(flex: 4,
                                child: Text('ITEM', style: AppText.label)),
                            SizedBox(width: 36,
                                child: Text('QTY', style: AppText.label,
                                    textAlign: TextAlign.center)),
                            SizedBox(width: 58,
                                child: Text('RATE', style: AppText.label,
                                    textAlign: TextAlign.right)),
                            SizedBox(width: 68,
                                child: Text('AMOUNT', style: AppText.label,
                                    textAlign: TextAlign.right)),
                          ]),
                        ),
                        ...
                        if (mergedItems.isEmpty)
                          ...
                          ...mergedItems.asMap().entries.map((entry) {
                            ...
                            return Container(
                              color: isEven ? Colors.white : const Color(0xFFFAFCFB),
                              padding: const EdgeInsets.symmetric(
                                  horizontal: 14, vertical: 10),
                              child: Row(children: [
                                Expanded(flex: 4,
                                    child: Text(i['item_name'] as String,
                                        style: const TextStyle(
                                            fontWeight: FontWeight.w600,
                                            fontSize: 13,
                                ...
                                SizedBox(width: 58,
                                    child: Text(_fmt.format(i['price'] as double),
                                        textAlign: TextAlign.right,
                                        style: const TextStyle(
                                            fontSize: 11, color: AppColors.textMid))),
                                SizedBox(width: 68,
                                    child: Text(_fmt.format(i['total'] as double),
                                        textAlign: TextAlign.right,
                                        style: const TextStyle(
                        ...
                        Container(
                          ...
                          child: Column(children: [
                            _totalRow('Subtotal', _fmt.format(totals['subtotal']!)),
                            const SizedBox(height: 6),
                             _totalRow('Tax (${RestaurantService.instance.taxRate.toStringAsFixed(0)}%)', _fmt.format(totals['gst']!)),
                            const Padding(
                    ...
                    Wrap(
                      ...
                      children: _methods.map((m) {
                        final sel = method == m;
                        final clr = _methodColors[m] ?? AppColors.primary;
                        return GestureDetector(
                          onTap: () => setDlg(() => method = m),
                          child: AnimatedContainer(
                            duration: const Duration(milliseconds: 180),
                            padding: const EdgeInsets.symmetric(
                                horizontal: 16, vertical: 11),
                            decoration: BoxDecoration(
                              color: sel ? clr : clr.withOpacity(0.06),
                              borderRadius: BorderRadius.circular(10),
                              border: Border.all(
                                  color: sel ? clr : clr.withOpacity(0.3),
                                  width: sel ? 2 : 1),
                              boxShadow: sel
                                  ? [BoxShadow(
                            ...
                            child: Row(mainAxisSize: MainAxisSize.min, children: [
                              Icon(_methodIcons[m],
                                  color: sel ? Colors.white : clr, size: 17),
              ...
              Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(
                    flex: 2,
                    child: StatefulBuilder(builder: (_, setBtnState) {
                      return ElevatedButton.icon(
                        onPressed: _collecting
                            ? null
                            : () async {
                                setBtnState(() => _collecting = true);
                                final ok = await _collectTablePayment(
                                ...
                                if (ok && mounted) {
                                  _collecting = false;
                                  Navigator.pop(ctx);
                                  _snack('✓ Payment collected for $tableNumber');
                                  await _load();
                                } else {
                                  setBtnState(() => _collecting = false);
                                }
                              },
                        icon: _collecting
                            ? const SizedBox(
  ...
  // ── REPRINT ──────────────────────────────────────────────────
  Future<void> _reprintBill(Map<String, dynamic> payment) async {
    ...
    final subtotal = amountPaid / 1.18;
    ...
    if (orderId != null) {
      try {
        final raw = await _sb.from('order_items').select().eq('order_id', orderId);
        items = List<Map<String, dynamic>>.from(raw);
      } catch (_) {}
    }
    ...
    for (final item in items) {
      ...
      if (merged.containsKey(key)) {
        ...
        merged[key] = {'item_name': name, 'quantity': qty, 'price': price, 'total': price * qty};
      }
    }
    ...
    await _generatePdfBill(
      ...
      totals: {'subtotal': subtotal, 'gst': gst, 'grandTotal': amountPaid},
      method: method,
  ...
  // ── TABLE CARD ───────────────────────────────────────────────
  Widget _buildTableCard(String tableNumber, List<Map<String, dynamic>> tableOrders) {
    final mergedItems = _mergeItemsFromOrders(tableOrders);
    final totals = _calculateTotals(mergedItems);
    final isParcel = tableNumber.startsWith('📦');

    return GestureDetector(
      onTap: () => _showTableBillDialog(tableNumber, tableOrders),
      child: Container(
              ...
              Text(_fmt.format(totals['grandTotal']!),
                  style: TextStyle(
                      ...
                      color: isParcel
                          ? const Color(0xFF7B3FF2)
                          : const Color(0xFF004D40))),
              const Text('incl. 18% GST',
                  style: TextStyle(fontSize: 10, color: AppColors.textMid)),
            ]),
            const Spacer(),
            ElevatedButton.icon(
              onPressed: () => _showTableBillDialog(tableNumber, tableOrders),
    ...
    if (_completedBills.isEmpty) {
      return Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const SizedBox(height: 6),
          const Text('Completed bills will appear here', style: AppText.body),
        ]),
      );
    }
    ...
    return ListView.builder(
      ...
      itemBuilder: (_, i) {
        ...
        final tableLabel = table != null
            ...
            : '#${order?['order_number'] ?? '—'}';
        final method = bill['payment_method'] as String? ?? 'cash';
        final amount = (bill['amount_paid'] as num?)?.toDouble() ?? 0;
        final dt = _parseUtc(bill['paid_at']); // ── FIX 2: use _parseUtc ──
        ...
        return Container(
          ...
          child: Row(children: [
            ...
            Expanded(child: Column(
              ...
              Row(children: [
                Icon(_methodIcons[method] ?? Icons.payments_outlined,
                    size: 12, color: clr),
...
Widget build(BuildContext context) {
  if (widget.tableKey != null && _hydratedTableOrders != null && !_loading && !_billDialogShown) {
          ...
          : Column(children: [
              Container(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                  ...
                  const SizedBox(height: 2),
                  const Text('Collect payments and manage invoices',
                      style: AppText.body),
                          ...
                          if (groupedOrders.isNotEmpty) ...[
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(
                                  ...
                                  borderRadius: BorderRadius.circular(10)),
                              child: Text('${groupedOrders.length}',
                                  style: const TextStyle(
                                      color: Colors.white,
                                      fontSize: 10,
                                      fontWeight: FontWeight.w800)),
                          ...
                          if (_completedBills.isNotEmpty) ...[
                            ...
                            Container(
                              padding: const EdgeInsets.symmetric(
                                  horizontal: 6, vertical: 1),
                              decoration: BoxDecoration(
                                  color: AppColors.textMid.withOpacity(0.15),
                                  borderRadius: BorderRadius.circular(10)),
                              child: Text('${_completedBills.length}',
                                  style: const TextStyle(
                                      color: AppColors.textMid,
                                      fontSize: 10,
                                      fontWeight: FontWeight.w800)),
                    ...
                    SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        LayoutBuilder(builder: (ctx, constraints) {
                          final cardCols = Responsive.isMobile(context) ? 1 : 3;
                          const spacing = 12.0;
                          final cardW =
                              (constraints.maxWidth - spacing * (cardCols - 1)) /
                                  cardCols;
                          return Wrap(
                              spacing: spacing,
                              runSpacing: spacing,
                              children: [
                            SizedBox(
                                width: cardW.clamp(120.0, 400.0),
                                child: _StatCard2(
                                    label: "Today's Revenue",
                                    value: _fmt.format(_todayRevenue),
                        ...
                        if (groupedOrders.isNotEmpty) ...[
                          Row(children: [
                            const Text('Ready for Billing', style: AppText.h2),
                            const SizedBox(width: 10),
                            Container(
                              padding: const EdgeInsets.symmetric(
                          ...
                          GridView.builder(
                            ...
                            itemBuilder: (_, i) {
                              final tableNum =
                                  groupedOrders.keys.elementAt(i);
                              final orders = groupedOrders[tableNum]!;
                              return _buildTableCard(tableNum, orders);
                            },
                          ),
                        ] else ...[
                          Container(
                            ...
                            child: Column(children: [
                              ...
                              const SizedBox(height: 14),
                              const Text('All cleared!', style: AppText.h4),
                              const SizedBox(height: 6),
                              const Text(
